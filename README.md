# SSIS Home Task
## Overview
This project demonstrates an ETL process using SQL Server Integration Services (SSIS) to load, transform, and incrementally update data from a staging table to a production table. 
The project includes ‘data cleaning’, ‘error handling’, and ‘logging’.
...
The package configurations are stored in a package_config table. It stores the details about the package, server and db used.
...
•	Data cleaning results are stored in another log tbl ‘RejectedRecords’ with their rejection reason.
...
3.	Data Loading
The data from the CSV file is loaded into a staging table. During this stage, initial data validation is performed to ensure that the data conforms to the expected db schema and SQL data types.
...
6.	Error Handling:
An ‘OnError’ Event Handler is implemented to record any errors that occur during the SSIS package execution. Errors are logged into the ‘audit_log’ table with detailed error messages and timestamps. Stored procedures include TRY-CATCH blocks to handle exceptions. In case of an error, the transaction is rolled back to maintain data integrity, and the error details are logged.
